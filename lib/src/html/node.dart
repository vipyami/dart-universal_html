...
abstract class _ElementOrDocument implements Node, ParentNode {
  ...
  void insertBefore(Node node, Node before) {
    ...
    if (!identical(node.ownerDocument, thisOwnerDocument) &&
        thisOwnerDocument != null) {
      throw new ArgumentError.value(node, "node",
          "Expected a node of type '${node.runtimeType}' to have 'ownerDocument' (${node.ownerDocument})");
    }
